...
function AnimatedSlider() {
  ...
  return (
    <div className="w-100 d-flex flex-column align-items-center" style={{ position: 'relative', height: '240px', overflow: 'hidden' }}>
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{ position: 'absolute', width: '100%', top: 0, left: 0, padding: '0 20px', textAlign: 'center' }}
        >
          <h2 style={{
            color: '#070f20',
            fontWeight: 700,
...
function VideoHero() {
  ...
  return (
    <div className="col-12 p-0 position-relative" style={{ height: '600px', overflow: 'hidden' }}>
      <video
        ref={videoRef}
        src="/next_assets/static/videos/bg-video-c35ede9e4b12eeaf34bf5d25e4548287.mp4"
      ...
      <button
        onClick={toggleMute}
        className="position-absolute bottom-0 end-0 m-3 btn btn-sm btn-light rounded-circle"
...
function Home() {
  const options = useParserOptions({
    replace: (domNode) => {
      if (domNode.attribs && domNode.attribs.id === 'VIDEO_HERO_PLACEHOLDER') {
        return <VideoHero />;
      }
